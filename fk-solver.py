...
if __name__ == '__main__':
    ...
    ## train and validate
    # load the dataset
    train_data_loader, test_data_loader, X_validate, y_validate, X_train, y_train, X_test, y_test = load_dataset_forward(data_a, n_DoF, batch_size, robot_choice, dataset_type, device, output_dim)
    ...
    # results_fkloss
    save_path = "results_forward/"+robot_choice+"/Forward_"+network_type+"_"+robot_choice+"_" \
    ...
    if save_option == "cloud":
        run = wandb.init(
            entity="jacketdembys",
            project = "ik-2",                
            group = network_type+"_"+"Forward_Dataset_"+str(dataset_samples)+"_Scale_"+str(int(scale))+"_"+dataset_type+"_"+loss_choice,  # "_seq", "_1_to_1"
            #group = "Dataset_Scale_"+str(int(scale)),
            name = "Forward_Model_"+robot_choice+"_" \
                    + save_layers_str + "_neurons_" + str(neurons) + "_batch_" + str(batch_size) +"_" \
    ...
    with torch.no_grad():
        results = forward_inference_modified(model, test_data_loader, criterion, device, robot_choice)
    ...
    y_errors_p = results["y_errors"]
    y_errors_p[:,:3] = y_errors_p[:,:3] * 1000
    y_errors_p[:,3:] = np.rad2deg(y_errors_p[:,3:]) 
    X_percentile = stats.percentileofscore(y_errors_p[:,0], [1,5,10,15,20], kind='rank')
    Y_percentile = stats.percentileofscore(y_errors_p[:,1], [1,5,10,15,20], kind='rank')
    Z_percentile = stats.percentileofscore(y_errors_p[:,2], [1,5,10,15,20], kind='rank')
    Ro_percentile = stats.percentileofscore(y_errors_p[:,3], [1,2,3,4,5], kind='rank')
    Pi_percentile = stats.percentileofscore(y_errors_p[:,4], [1,2,3,4,5], kind='rank')
    Ya_percentile = stats.percentileofscore(y_errors_p[:,5], [1,2,3,4,5], kind='rank')
    ...
    if save_option == "cloud":
        inference_results = {
            ...
            "min_x(mm)": y_errors_r[0,0],
            "avg_x(mm)": y_errors_r[1,0],
            "max_x(mm)": y_errors_r[2,0],
            "std_x(mm)": y_errors_r[3,0],
            ...
            "min_y(mm)": y_errors_r[0,1],
            "avg_y(mm)": y_errors_r[1,1],
            "max_y(mm)": y_errors_r[2,1],
            "std_y(mm)": y_errors_r[3,1],
            ...
            "min_z(mm)": y_errors_r[0,2],
            "avg_z(mm)": y_errors_r[1,2],
            "max_z(mm)": y_errors_r[2,2],
            "std_z(mm)": y_errors_r[3,2],
            ...
            "min_ro(deg)": y_errors_r[0,3],
            "avg_ro(deg)": y_errors_r[1,3],
            "max_ro(deg)": y_errors_r[2,3],
            "std_ro(deg)": y_errors_r[3,3],
            ...
            "min_pi(deg)": y_errors_r[0,4],
            "avg_pi(deg)": y_errors_r[1,4],
            "max_pi(deg)": y_errors_r[2,4],
            "std_pi(deg)": y_errors_r[3,4],
            ...
            "min_ya(deg)": y_errors_r[0,5],
            "avg_ya(deg)": y_errors_r[1,5],
            "max_ya(deg)": y_errors_r[2,5],
            "std_ya(deg)": y_errors_r[3,5],
