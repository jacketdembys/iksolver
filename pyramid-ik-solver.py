...
if __name__ == '__main__':
    ...
    if network_type == "PMLP":
        models = []
        for i in range(output_dim):
            models.append(PMLP(input_dim, hidden_layers, hidden_neurons, output_dim))
    ...
    if save_option == "cloud":
        run = wandb.init(
            ...
            name = "Modif_Err_2_Model_"+robot_choice+"_" \
                    + save_layers_str + "_neurons_" + str(neurons) + "_batch_" + str(batch_size) +"_" \
    ...
    if network_type == "PMLP":
        model = PMLP(input_dim, hidden_layer_sizes, output_dim).to(device)
