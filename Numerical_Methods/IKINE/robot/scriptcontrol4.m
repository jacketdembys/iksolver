...
for i=1:niters
   ...
   % x1e=x1next;
    ...
    SdX1e(i,:)=(x1next-x1e)'/h;
            ...
            if i>5
                %x1e=0.2*(x1e+SX1e(i-1,:)'+SX1e(i-2,:)'+SX1e(i-3,:)'+SX1e(i-4,:)');
            end
            x2old=x2e;
            x2e=C*x1e+D*[q;dq];
            x2e=(1-a)*x2e+a*x2old;
    ...
    if i>5
        Suext2=[Suext2;1/15*(5*Suext(i-1,:)+4*Suext2(i-2,:)+3*Suext2(i-3,:)+2*Suext2(i-4,:)+Suext2(i-5,:))]; 
    else
        Suext2=Suext;
    end
...
%                plot(time(1:size(time,1)-1),Suext)
                ...
                %plot(time,SddQ,'Linewidth',2)
                hplot2=plot(time(1:size(time,1)-casos),Suext2(:,1),'b');
                
                hold on
                 %plot(time(1:size(time,1)),SdX2e(:,1),'b')
                plot(time(1:size(time,1)-casos),Suext2(:,2),'r')
                plot(time(1:size(time,1)-casos),Suext2(:,3),'g')
                plot(time(1:size(time,1)-casos),Suext2(:,4),'y')
                plot(time(1:size(time,1)-casos),Suext2(:,5),'m')
                plot(time(1:size(time,1)-casos),Suext2(:,6),'c')
                plot(time(1:size(time,1)-casos),Suext2(:,7),'k')
                %legend('q1','q2','q3','q4','q5','q6')
                titol=strcat('k11=',num2str(K11(1,1)),',k12=',num2str(K12(1,1)),',k21=',num2str(K21(1,1)),',k22=',num2str(K22(1,1)));
